...
{
  if ((millis() < endMillis2) && ((lastPedalName[0] != ':') && (lastPedalName[0] != ';')) ) {
      ...
      if (banknames[currentBank][0] == 0) {
        ...
        for (byte p = 0; p < 9; p++) {
          ...
          if ( pedal_x[p] >= 0 ) { // not for pedals 5 & 6
            bool invertText = true;
            if ((banks[currentBank][p].pedalName[0] == ';') || (banknames[currentBank][0] == ';')) {
              invertText = false;
            }
            name = String(((banks[currentBank][p].pedalName[0] == ':') || (banks[currentBank][p].pedalName[0] == ';') ) ? &banks[currentBank][p].pedalName[1] : banks[currentBank][p].pedalName);
        ...
        // Center area
        if (((millis() - ms < 4000) && (banknames[currentBank][0] != '.')) || ((banknames[currentBank][0] == ':') || (banknames[currentBank][0] == ';'))) {
          // Display bank name
          display->drawRect(0, 23, 128, 28);
          name = ((banknames[currentBank][0] == ':') || (banknames[currentBank][0] == ';')) ? &banknames[currentBank][1] : banknames[currentBank];
        ...
        else if (((millis() - ms < 8000) || (banknames[currentBank][0] == '.')) && ((banknames[currentBank][0] != ':') && (banknames[currentBank][0] != ';'))) {
          ...
          for (byte p = 0; p < 9; p++) {
            if ( pedal_x[p] >= 0 ) { // not for pedals 5 & 6
              if (IS_SINGLE_PRESS_ENABLED(pedals[p].pressMode) && (banks[currentBank][p].midiMessage != PED_EMPTY)) {
                display->drawProgressBar(pedal_x[p] + 2 + x, pedal_y1[p] + y, 39, 11, constrain(map2(currentMIDIValue[currentBank][p][0],
...
void display_update() {
  ...
  if (uiUpdate && !reloadProfile) {
    ...
    bool showPedalInfo = ((millis() < endMillis2) && ((lastPedalName[0] != ':') && (lastPedalName[0] != ';')));
